...
#define QUERY_FOLDER "/home/dlsu-cse/Downloads/Testing2026/Testing/queries"
#define REFERENCE_FOLDER "/home/dlsu-cse/Downloads/Testing2026/Testing/references"
#define RESULTS_FOLDER "results"
#define RESULTS_CSV_FILE "results/Cresults.csv"
#define LOOP_COUNT 1
...
/* --- Directory and CSV helpers --- */
int create_result_folder() {
    struct stat st = {0};
    if (stat(RESULTS_FOLDER, &st) == -1) {
        if (mkdir(RESULTS_FOLDER, 0700) == -1) {
            perror("Failed to create result folder");
            return 0;
        }
    }
    return 1;
...
char** get_files_from_folder(const char* folder, int* count) {
...
char* format_indices(int* indices, int count) {
    if (count == 0) return strdup("N/A");
    
    char* result = malloc(count * 12 + 1);
    result[0] = '\0';
    for (int i = 0; i < count; i++) {
        char buffer[20];
        sprintf(buffer, "%d", indices[i]);
...
int main(){
    ...
    if (!query_files || !ref_files || num_query_files == 0 || num_ref_files == 0) {
        fprintf(stderr, "No query or reference files found\n");
        return 1;
    }
    ...
    // Write CSV header
    fprintf(csv_file, "C Run\n");
    fprintf(csv_file, "Query File,Query Length,Reference File,Reference Length,Number of Hits,Hit Indexes,Lowest Score,Lowest Score Indexes,Last Score\n");
    ...
    for (int loop = 0; loop < LOOP_COUNT; loop++) {
        ...
        // Process each query against each reference
        for (int q = 0; q < num_query_files && q < 2; q++) {
            char query_path[512];
            sprintf(query_path, "%s/%s", QUERY_FOLDER, query_files[q]);
            
            int num_queries = 0;
            char** queries = parse_fasta_file(query_path, &num_queries);
            if (!queries) {
                fprintf(stderr, "Failed to load query file: %s\n", query_files[q]);
                continue;
            }

            for (int r = 0; r < num_ref_files && r < 2; r++) {
                char ref_path[512];
                sprintf(ref_path, "%s/%s", REFERENCE_FOLDER, ref_files[r]);
                
                int num_refs = 0;
                char** refs = parse_fasta_file(ref_path, &num_refs);
                if (!refs) {
                    fprintf(stderr, "Failed to load reference file: %s\n", ref_files[r]);
                    continue;
                }

                // Process each query-reference pair
                for (int qi = 0; qi < num_queries; qi++) {
                    for (int ri = 0; ri < num_refs; ri++) {
                        int query_len = strlen(queries[qi]);
                        int ref_len = strlen(refs[ri]);
                        int* scores = malloc(ref_len * sizeof(int));
                        int *hit_idxs = NULL, hit_count = 0;
                        int *low_idxs = NULL, low_count = 0, lowest = INT_MAX;

                        int dist = bit_vector_damerau(queries[qi], refs[ri], scores,
                                                       &hit_idxs, &hit_count,
                                                       &low_idxs, &low_count, &lowest);

                        if (loop == 0) {
                            // Format indices
                            char* hit_indices = format_indices(hit_idxs, hit_count);
                            char* lowest_indices = format_indices(low_idxs, low_count);
                            int last_score = (ref_len > 0) ? scores[ref_len - 1] : -1;

                            // Build lowest score string (N/A if there are hits)
                            char lowest_score_str[20];
                            if (hit_count > 0) strcpy(lowest_score_str, "N/A");
                            else sprintf(lowest_score_str, "%d", lowest);

                            // Write to CSV (index fields are quoted to handle commas)
                            fprintf(csv_file, "%s,%d,%s,%d,%d,\"%s\",%s,\"%s\",%d\n",
                                    query_files[q], query_len,
                                    ref_files[r], ref_len,
                                    hit_count, hit_indices,
                                    lowest_score_str,
                                    (hit_count > 0) ? "N/A" : lowest_indices,
                                    last_score);

                            // Print to console
                            printf("----------------------------------------------------------------------------\n");
                            printf("Pair: %s(%d) Vs %s(%d)\n",
                                   query_files[q], query_len,
                                   ref_files[r], ref_len);
                ...
                for (int i = 0; i < num_refs; i++) free(refs[i]);
                free(refs);
            }
    ...
    fclose(csv_file);
    free_files(query_files, num_query_files);
    free_files(ref_files, num_ref_files);
