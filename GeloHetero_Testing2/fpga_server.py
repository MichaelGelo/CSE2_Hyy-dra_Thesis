...
def free_mem():
    ...
    cma = subprocess.check_output(
        "cat /proc/meminfo | grep 'CmaFree'", shell=True
    ).decode().strip()

    print(f"    {mem}")
    print(f"    {cma}")

    return int(mem.split()[1])  # in kB


def recv_exact_into(sock, size, view):
    """Receive exactly 'size' bytes into a memoryview using balanced chunks."""
    recvd = 0
    chunk_size = 256 * 1024  # 256KB chunks - balanced for throughput
    while recvd < size:
...
def run_server():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        # Massive buffers for high-throughput
        ...
        while True:
            ...
            with conn:
                try:
                    ...
                    # Allocate CMA buffers for EXACT sizes needed
                    ...
                    # Receive DIRECTLY into CMA buffers (no copy!)
                    ...
                    results = hyyrorunner.run_hyyro_buffers(
                        query_cma, ref_cma
                    )
