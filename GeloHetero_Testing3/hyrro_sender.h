...
static int send_all(int sock, const void *data, size_t size) {
    ...
    size_t chunk_size = 256 * 1024; // 256KB chunks - balance throughput vs latency
    while (remaining > 0) {
        size_t to_send = (remaining > chunk_size) ? chunk_size : remaining;
        ssize_t sent = send(sock, ptr, to_send, MSG_NOSIGNAL);
        if (sent <= 0) return -1;
        ptr += sent;
        remaining -= sent;
    }
...
static FPGAResult run_single_fpga(const char* ref_file, const char* query_file) {
    ...
    // ========== Read MemFree before allocations ==========
    long mem_before = read_memfree_kb();
    printf("[FPGA] MemFree before allocation: %ld kB\n", mem_before);
    ...
    int sndbuf = 256 * 1024 * 1024; // 256MB for burst throughput
    ...
    // ========== SEND ALL DATA IN ONE STREAM ==========
    // Send header + query + reference as continuous stream (no separate calls)
    int header[2] = { query_len, ref_len };
    
    struct timespec ts_start, ts_end;
    clock_gettime(CLOCK_MONOTONIC, &ts_start);
    // Send header, query, and reference back-to-back
    send_all(sock, header, sizeof(header));
    send_all(sock, query_seq, query_len);
    send_all(sock, ref_seq, ref_len);
    
    clock_gettime(CLOCK_MONOTONIC, &ts_end);
    double total_send_time = (ts_end.tv_sec - ts_start.tv_sec) + (ts_end.tv_nsec - ts_start.tv_nsec) / 1e9;
    double total_bytes = sizeof(header) + query_len + ref_len;
    double total_mbps = (total_bytes * 8.0) / (total_send_time * 1e6);

    printf("[FPGA] Total sent: %.2f MB in %.4f sec → %.2f Mbps\n",
           total_bytes / (1024*1024), total_send_time, total_mbps);
    ...
    clock_gettime(CLOCK_MONOTONIC, &ts_end);
    result.total_exec_ms = ((ts_end.tv_sec - ts_start.tv_sec) + (ts_end.tv_nsec - ts_start.tv_nsec) / 1e9) * 1000.0;
