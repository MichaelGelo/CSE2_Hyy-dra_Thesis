...
static float calculate_gpu_ratio(double gpu_time, double fpga_time, float old_ratio) {
    ...
    // === Dead-band: suppress tiny corrections caused by timing noise ===
    // If GPU and FPGA times are within RATIO_DEAD_BAND of each other, the
    // imbalance is likely noise — keep the current ratio to prevent drift.
    double time_imbalance = fabs(fpga_time - gpu_time) / fmax(fpga_time, gpu_time);
    if (time_imbalance < RATIO_DEAD_BAND) {
        return old_ratio;
    }
