    ...
    size_t totalRefBytes)  // NEW: Total actual bytes needed for all chunks
{
    ...
    float eq_mb = (numQueries * 256 * sizeof(bv_t)) / (1024.0 * 1024.0);
    float q_mb = (numQueries * MAX_QUERY_LENGTH) / (1024.0 * 1024.0);
    float r_mb = totalRefBytes / (1024.0 * 1024.0);
    float total_mb = (numQueries * 256 * sizeof(bv_t) + 
...
static inline void packSequencesEfficient(
    ...
    size_t* totalQueryBytes, size_t* totalRefBytes)
{
    ...
    *totalQueryBytes = (size_t)numQueries * alignedQueryLen * sizeof(char);
    *hostQueries = (char*)calloc(*totalQueryBytes, 1);
    
    printf("[GPU] Query packing: length=%d, aligned=%d, slots=%d, total=%.1f KB\n",
           actualQueryLen, alignedQueryLen, numQueries, *totalQueryBytes / 1024.0);
    ...
    *hostRefOffsets = (int*)malloc(numChunks * sizeof(int));
    ...
    printf("[GPU] Packed %d chunks (%.1f MB, avg %.1f KB/chunk)\n",
    ...
    size_t totalRefBytes)
{
    ...
    CUDA_CHECK(cudaMemcpy(buffers->d_queries, hostQueries,
                         totalQueryBytes, cudaMemcpyHostToDevice));
