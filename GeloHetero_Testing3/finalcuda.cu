...
int main() {
    ...
    // ========== Load single query and single reference ==========
    int num_queries = 0;
    char** query_seqs = load_queries_from_file(QUERY_FILE, &num_queries);
    if (!query_seqs || num_queries == 0) {
        fprintf(stderr, "Failed to load query from %s\n", QUERY_FILE);
        return EXIT_FAILURE;
    }
    ...
    int num_orig_refs = 0;
    char** orig_refs = load_references_from_file(REFERENCE_FILE, &num_orig_refs);
    if (!orig_refs || num_orig_refs == 0) {
        fprintf(stderr, "Failed to load reference from %s\n", REFERENCE_FILE);
        ...
        return EXIT_FAILURE;
            ...
            float*  ref_gpu_ratios_used  = (float*) malloc(num_orig_refs * sizeof(float));
            float*  ref_fpga_ratios_used = (float*) malloc(num_orig_refs * sizeof(float));
            double* ref_gpu_times        = (double*)malloc(num_orig_refs * sizeof(double));
            double* ref_fpga_times       = (double*)malloc(num_orig_refs * sizeof(double));
            ...
            for (int ref_idx = 0; ref_idx < num_orig_refs; ref_idx++) {
                ...
                ref_gpu_ratios_used[ref_idx]  = gpu_ratio;
                ref_fpga_ratios_used[ref_idx] = fpga_ratio;
                ...
                ref_gpu_times[ref_idx]  = gpu_time;
                ref_fpga_times[ref_idx] = fpga_time;
            ...
            printf("\n========================================================================\n");
            printf("Summary: All %d pair(s) processed with adaptive ratios\n", num_queries * num_orig_refs);
            printf("Total GPU time:  %.6f sec\n", total_gpu_time);
            printf("Total FPGA time: %.6f sec\n", total_fpga_time);
            printf("\nRatios Per Reference:\n");
            for (int r = 0; r < num_orig_refs; r++) {
                printf("Ref%d used:   GPU %.3f FPGA %.3f\n",
                       r + 1, ref_gpu_ratios_used[r], ref_fpga_ratios_used[r]);
                printf("Ref%d times:  GPU %.6f sec, FPGA %.6f sec\n",
                       r + 1, ref_gpu_times[r], ref_fpga_times[r]);
                if (r < num_orig_refs - 1) printf("\n");
            }
            printf("\nFinal GPU ratio: %.3f, FPGA ratio: %.3f\n", gpu_ratio, fpga_ratio);
            printf("========================================================================\n");
            ...
            free(ref_gpu_ratios_used);
            free(ref_fpga_ratios_used);
            free(ref_gpu_times);
            free(ref_fpga_times);
