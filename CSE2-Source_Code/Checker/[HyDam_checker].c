...
#define RESULTS_CSV_FILE "results/Cresults2.csv"
#define LOOP_COUNT 1
...
char** get_files_from_folder(const char* folder, int* count) {
    ...
    if (n == 0) { free(files); *count = 0; return NULL; }

    qsort(files, (size_t)n, sizeof(char*),
...
int main(){
    ...
    for (int loop = 0; loop < LOOP_COUNT; loop++) {
        ...
        for (int q = 0; q < num_query_files; q++) {
            char query_path[512];
            ...
            for (int r = 0; r < num_ref_files; r++) {
                ...
                for (int qi = 0; qi < num_queries; qi++) {
                    for (int ri = 0; ri < num_refs; ri++) {
                        ...
                        bit_vector_damerau(queries[qi], refs[ri], scores,
                                                       &hit_idxs, &hit_count,
